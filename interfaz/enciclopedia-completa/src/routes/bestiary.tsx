...
import { stages, dossierSections, finalRecord, scaleRefs } from "@/data/bestiary-data";
import { Dossier } from "@/components/bestiary/Dossier";
import {
  BiologicalMatrix, HabitatHub, BehaviorLedger, LoreCodex, GeneticForge, OmniSearch,
} from "@/components/bestiary/AdvancedModules";
import { WorldCodex } from "@/components/bestiary/WorldCodex";
import {
...
export const Route = createFileRoute("/bestiary")({
  head: () => ({
    meta: [
      { title: "吞噬星空 · Enciclopedia Premium" },
      {
        name: "description",
        content:
          "Enciclopedia ultra-premium del Soberano del Cuerno Dorado — fusión visual, glass premium, analítica ejecutiva y matriz de comando.",
      },
    ],
    links: [
    ...
    <div className="relative min-h-screen overflow-hidden" onMouseMove={onPageMove}>
      ...
      <header className="fixed top-0 inset-x-0 z-40 px-6 md:px-10 py-3.5 flex justify-between items-center bg-gradient-to-b from-background/85 to-transparent">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 text-center pointer-events-none">
          <div className="cn-title text-foreground/95 text-xl md:text-2xl tracking-[0.6em]">吞噬星空</div>
          <div className="tech text-[10px] text-foreground/50 tracking-[0.8em] mt-1">
            S W A L L O W E D · S T A R
          </div>
        </div>
      ...
      <main className="relative min-h-screen pt-20 pb-32">
        ...
        <div className="absolute z-10 bottom-44 right-6 md:right-10 text-right hidden md:block">
          <div className="cn-title text-gold/90 text-base tracking-[0.4em] leading-tight">吞噬星空</div>
          <div className="tech text-[9px] text-gold/50 tracking-[0.4em] mt-1">ENCICLOPEDIA · {stage.chapter}</div>
        </div>
      </main>

      {/* ░░░ CONTEXTUAL ASSET LEDGER ░░░ */}
      <section className="relative z-10 px-6 md:px-10 pb-10">
        <div className="flex items-end justify-between mb-4">
          <div>
            <div className="flex items-center gap-2">
              <Layers size={12} className="text-gold" />
              <span className="tech text-[10px] text-gold tracking-[0.4em]">CONTEXTUAL ASSET LEDGER</span>
            </div>
            <h2 className="cn-title text-lg md:text-xl text-foreground/95 mt-1">Galería del Soberano</h2>
          </div>
